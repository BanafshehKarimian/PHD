...
from models.model_vmra_v0 import *
import torch
...
model_dict = {"LoMaR": LoMaRMasked, "VMRA": VMRAMasked}
...
def history_impute_loss(pred_hist, true_hist, true_hist_mask, mode="mse"):
    """
    pred_hist:      [B,4,D]
    true_hist:      [B,4,D]
    true_hist_mask: [B,4]  (1=available, 0=truly missing)
    """
    m = true_hist_mask.unsqueeze(-1).float()  # [B,4,1]
    if mode == "mse":
        per = (pred_hist - true_hist).pow(2)  # [B,4,D]
        loss = (per * m).sum() / m.sum().clamp(min=1.0)
        return loss
    elif mode == "cos":
        # cosine per timestep
        cos = 1.0 - F.cosine_similarity(pred_hist, true_hist, dim=-1)  # [B,4]
        loss = (cos * true_hist_mask.float()).sum() / true_hist_mask.sum().clamp(min=1.0)
        return loss
    else:
...
def cosine_feature_loss(pred_f, true_f, eps=1e-8, reduction="mean"):
    """
    pred_f, true_f: [B, H] (or [B, *] flattened on last dim)
    Returns: scalar loss
    """
    pred_n = F.normalize(pred_f, dim=-1, eps=eps)
    true_n = F.normalize(true_f, dim=-1, eps=eps)

    # cosine similarity per sample
    cos = (pred_n * true_n).sum(dim=-1)          # [B]
    loss = 1.0 - cos                             # [B]
    ...
    if reduction == "sum":
        return loss.sum()
    return loss
def kd_bce_binary(student_logits, teacher_logits, mask=None, T=4.0, reduction="mean"):
    """
    student_logits, teacher_logits: [B] or [B,1] logits
    mask: [B] float/bool, 1=use, 0=ignore (e.g., unknown labels)
    """
    s = student_logits.view(-1)
    t = teacher_logits.view(-1)

    # teacher soft target with temperature
    with torch.no_grad():
        p_t = torch.sigmoid(t / T)

    # student probability at same temperature
    log_p_s = F.logsigmoid(s / T)            # log(sigmoid(.))
    log_1mp_s = F.logsigmoid(-(s / T))       # log(1-sigmoid(.))

    # cross-entropy with soft targets (equiv to Bernoulli KL up to const)
    loss = -(p_t * log_p_s + (1.0 - p_t) * log_1mp_s)   # [B]

    # scale like standard KD
    loss = loss * (T * T)
...
def get_loss(outputs, labels, out, pos_weight=None, alpha_f = 0.1, alpha_l = 0.1, tmp = 4):
    """
    outputs: [B, K] logits
    ...
    loss += alpha_l * kd_bce_binary(
        ...
        teacher_logits=out["logits"]["true"][0][:, k],
        mask=label_mask[:, k],
        T=tmp
    )
    k = 3
    loss += alpha_l * kd_bce_binary(
        student_logits=out["logits"]["pred"][:, k],
        teacher_logits=out["logits"]["true"][1][:, k],
        mask=label_mask[:, k],
        T=tmp
    )
    loss += alpha_l * kd_bce_binary(
        student_logits=out["logits"]["pred"][:, 0],
        teacher_logits=out["logits"]["true"][2][:, 0],
        mask=label_mask[:, k],
        T=tmp
    )
    loss += alpha_l * kd_bce_binary(
        student_logits=out["logits"]["pred"][:, 1],
        teacher_logits=out["logits"]["true"][2][:, 1],
        mask=label_mask[:, k],
        T=tmp
    )
    loss += alpha_l * kd_bce_binary(
        student_logits=out["logits"]["pred"][:, 2],
        teacher_logits=out["logits"]["true"][2][:, 2],
        mask=label_mask[:, k],
        T=tmp
    )
    #loss +=  alpha_l*kd_loss_multilabel(out["logits"]["pred"][:,-2:], out["logits"]["true"][:,-2:], mask = label_mask[:,-2:], temperature = 8)
    #loss_f = 1.0 - F.cosine_similarity(out["pred_f"], out["true_f"], dim=-1)
    #loss +=  alpha*loss_f.sum()
    return loss
...
def get_example_config():  # without density, ver_1 data, visit aug off, no risk, with long early stop, survival.
    ...
    p.add_argument("--alpha-l", type=float, default=0.1)
    p.add_argument("--alpha-f", type=float, default=0.1)
    p.add_argument("--tmp", type=float, default=4)
    ...
    p.add_argument("--path_t1", type=str, default="")
    p.add_argument("--path_t2", type=str, default="")
    p.add_argument("--path_t3", type=str, default="")
    ...
    config = {
        ...
        "path_t2": args.path_t2,
        "path_t3": args.path_t3,
    }
    return config
...
def train(model, train_loader, val_loader, config):
    run_dir = f'{config["exp_id"]}_{config["history_masking_id"]}_{config["init_lr"]}_{config["weight_decay"]}_{config["model_embedding_dim"]}_{config["n_heads"]}_{config["alpha_l"]}_{config["alpha_f"]}_{config["tmp"]}'
    ...
    optimizer = torch.optim.Adam(
        model.parameters(),#list(model.visit_transformer.parameters()) + list(model.classifier.parameters()),
        lr=config["init_lr"],
    ...
    for ep in range(config['train_epoch']):
        model.train()
        for teacher in model.teacher_branch:
            teacher.eval()
        #model.imputer.eval()
        loss_log = 0.0
        ep_pred, ep_label = [], []
        ...
        for batch in tqdm(train_loader):
            ...
            outputs, out = model(batch)                         # [B, K]

            loss = get_loss(outputs, labels, out, pos_weight=pos_weight, alpha_l = config['alpha_l'], alpha_f = config['alpha_f'], tmp = config['tmp'])
            #c = cosine_feature_loss(out["pred_f"][:,:3],out["true_f"][:,:3])
            #loss += c
            #print(c)
        ...
        for batch in tqdm(val_loader):
            ...
            with torch.no_grad():
                ...
                loss = get_loss(outputs, labels, out, pos_weight=pos_weight, alpha_l = 0, alpha_f = 0)
                #loss += cosine_feature_loss(out["pred_f"],out["true_f"])
...
def run_train(config):
    ...
    model.add_teacher_model(config["path_t1"])
    model.add_teacher_model(config["path_t2"])
    model.add_teacher_model(config["path_t3"])
    model.load_imputer(f'./results/imp_mse/{config["exp_id"]}_train/best.pth')
    

    history_masking_id = config["history_masking_id"]
    train_dataset =  BreastDataset(config, history_masking_id, split="train", random_masking = False, curriculum_masking=True)
    val_dataset =  BreastDataset(config, history_masking_id, split="val", random_masking = False, curriculum_masking=True)
    val_dataset.mask_indx = 4 
    train_dataset.mask_indx = 4 
...
def run_test(path, config, fpr = None):
    ...
    history_masking_id = config["history_masking_id"]
    test_dataset =  BreastDataset(config, history_masking_id, split="test", random_masking = False, curriculum_masking=True)
    test_dataset.mask_indx = 4
...
Lrate = [config_tmp["weight_decay"]]#[1e-4, 1e-5, 1e-6]#
for d in d_list:
    for h in n_heads:
        for l in Lrate:
