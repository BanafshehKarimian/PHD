...
from models.model_vmra import VMRA
...
class VMRAMasked(nn.Module):
    def __init__(self, config, student = False):
        super(VMRAMasked, self).__init__()
    ...
    def forward(self, batch, visit_key = 'visit_embeddings', mask_key = 'visit_mask'):
        ...
        #pred_hist = torch.zeros_like(pred_hist)
        pred_full = torch.cat([pred_hist, x0.unsqueeze(1)], dim=1)
        '''x0 = orig[:, -1, :]                    # [B,D]
        with torch.no_grad():
            pred_hist = self.imputer(x0)           # [B,4,D]
        pred_hist = pred_hist.view(orig.shape[0], 4, orig.shape[2])#self.mapper()
        fused_feats = torch.cat([pred_hist, x0.unsqueeze(1)], dim=1)'''
        fused_feats = pred_full * (1-mask).unsqueeze(-1)
        states = None
        vmrnn_outputs = []
        ...
        return risk_prediction_logits, {"mask": 1.0 - mask[:,:-1], "pred": pred_hist, "true": orig[:,:-1], "logits": logits, "true_f":complete_history_embedding_, "pred_f": hidden_states}
